...
- Интеграция с локальным `agent-api` + `Ollama` на Mac или с Hermes как legacy-вариант.
...
Основной вариант сейчас: `agent-api` на твоем Mac + `Ollama`.

Как это работает:

- Новый бот читает анкету из Supabase.
- Анкеты, `/report`, ежедневные отчеты и `/templates` отправляются в `agent-api`.
- `agent-api` обращается к `Ollama` и возвращает оценку, выводы, риски, рекомендации и шаблоны.
- Локальные формулы используются как аварийный fallback.

Переменные:

```env
AI_PROVIDER=agent_api
AGENT_API_BASE_URL=https://your-agent-api.ngrok-free.app
AGENT_API_KEY=
...
- `/warn <user_id|@username> [reason]` — предупреждение.
- `/unwarn <user_id|@username>` — снять одно предупреждение.
- `/mute <user_id|@username> [minutes]` — мут.
- `/unmute <user_id|@username>` — снять мут.
- `/kick <user_id|@username>` — кик (с возможностью вернуться по ссылке).
- `/ban <user_id|@username>` — перманентный бан.
- `/unban <user_id|@username>` — разбан.

Также эти команды можно использовать ответом на сообщение.
