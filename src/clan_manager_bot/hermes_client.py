...
class HermesClient:
    ...
    async def score_candidate(self, data: ScoringInputs, fallback: CandidateScore) -> CandidateScore:
        if self.settings.agent_api_enabled:
            try:
                content = await self._agent_chat(self._build_agent_candidate_prompt(data, fallback))
                parsed = parse_hermes_candidate_json(content)
                return merge_hermes_candidate_score(data, fallback, parsed)
            except Exception:
                logger.exception("Agent API candidate scoring failed; using local fallback")
                return fallback
    ...
    async def build_clan_report(self, items: list[MemberAnalytics], fallback: str) -> str:
        if self.settings.agent_api_enabled:
            try:
                content = await self._agent_chat(self._build_agent_report_prompt(items))
                return _clean_report_text(content)
            except Exception:
                logger.exception("Agent API clan report failed; using local fallback")
    ...
    async def build_newbie_templates(self, messages: list[str], fallback: list[str]) -> list[str]:
        if self.settings.agent_api_enabled:
            try:
                content = await self._agent_chat(self._build_agent_templates_prompt(messages))
                parsed = parse_hermes_candidate_json(content)
                templates = _clean_text_list(parsed.get("templates"))
                return templates[:8] or fallback
            except Exception:
                logger.exception("Agent API templates generation failed; using local fallback")
                return fallback
    ...
    async def ask_assistant(self, prompt: str, fallback: str) -> str:
        if self.settings.agent_api_enabled:
            try:
                content = await self._agent_chat(prompt)
                text = content.strip()
                return text[:3900] if text else fallback
            except Exception:
                logger.exception("Agent API assistant reply failed; using fallback")
                return fallback
        ...
        if self.settings.hermes_enabled:
            try:
                payload = {
                    ...
                    "temperature": 0.3,
                    "max_tokens": min(self.settings.hermes_max_tokens, 700),
                    "messages": [
                        {
                            "role": "system",
                            "content": "Ты краткий ассистент админов клана Car Parking Multiplayer 2. Отвечай по-русски, коротко и по делу.",
                        },
                ...
                logger.exception("Hermes assistant reply failed; using fallback")
                return fallback
    ...
    async def _agent_chat(self, prompt: str) -> str:
        ...
        base_url = str(self.settings.agent_api_base_url).rstrip("/")
        async with httpx.AsyncClient(timeout=self.settings.agent_timeout_seconds) as client:
            response = await client.post(f"{base_url}/chat", headers=headers, json={"prompt": prompt})
            response.raise_for_status()
            body = response.json()
    ...
    def _build_templates_payload(self, messages: list[str]) -> dict[str, Any]:
        return {
            "model": self.settings.hermes_model,
            "temperature": 0.3,
            "max_tokens": min(self.settings.hermes_max_tokens, 900),
            "messages": [
                {
                    "role": "system",
                    "content": (
    ...
    def _build_agent_candidate_prompt(self, data: ScoringInputs, fallback: CandidateScore) -> str:
        payload = {
            "task": "candidate_application_score",
            "scale": "0-100",
            "local_fallback_score": fallback.score,
        ...
        return (
            "Ты AI-офицер клана Car Parking Multiplayer 2. "
            "Оцени анкету кандидата строго и коротко. "
            "Ответь только JSON без markdown и без пояснений.\n\n"
            + json.dumps(payload, ensure_ascii=False)
        )

    def _build_agent_report_prompt(self, items: list[MemberAnalytics]) -> str:
        payload = {
            "task": "weekly_clan_chat_analytics",
            "members": [member_analytics_to_dict(item) for item in items[:100]],
            "required_sections": [
        ...
        return (
            "Ты AI-аналитик клана Car Parking Multiplayer 2. "
            "Дай понятный отчет для админов: активность, дисциплина, кандидаты в админы, "
            "риски и конкретные действия. Пиши по-русски, коротко, без markdown-таблиц.\n\n"
            + json.dumps(payload, ensure_ascii=False)
        )

    def _build_agent_templates_prompt(self, messages: list[str]) -> str:
        payload = {
            "task": "newbie_reply_templates",
            "messages": messages[:200],
            "required_json": {"templates": ["4-8 short ready-to-send replies"]},
