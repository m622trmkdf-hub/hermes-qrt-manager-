...
class BotRuntime:
    ...
    async def cmd_warn(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not user_id:
            await update.effective_message.reply_text("Использование: /warn <user_id> <причина> или ответом на сообщение")
            return
    ...
    async def cmd_mute(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        user_id = await self._extract_target_user(update, context)
        if not user_id:
            await update.effective_message.reply_text("Использование: /mute <user_id> <минуты>")
            return
    ...
    async def cmd_kick(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        user_id = await self._extract_target_user(update, context)
        if not user_id:
            await update.effective_message.reply_text("Использование: /kick <user_id>")
            return
    ...
    async def cmd_ban(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        user_id = await self._extract_target_user(update, context)
        if not user_id:
            await update.effective_message.reply_text("Использование: /ban <user_id>")
            return
