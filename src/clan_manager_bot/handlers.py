...
class BotRuntime:
    ...
    async def cmd_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.effective_message.reply_text(
            "Умный менеджер клана активен. Команды: /report, /templates, /warn, /unwarn, /mute, /unmute, /kick, /ban, /unban"
        )

    def _is_assistant_chat_allowed(self, update: Update) -> bool:
        ...
        if chat.type == "private":
            return True
        if chat.id == self.settings.public_chat_id:
            return True
        if chat.id == self.settings.admin_chat_id:
    ...
    async def cmd_unwarn(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        if not await self.admin_guard(update):
            return

        user_id = await self._extract_target_user(update, context)
        if not user_id:
            await update.effective_message.reply_text("Использование: /unwarn <user_id|@username> или ответом на сообщение")
            return

        actor_id = update.effective_user.id if update.effective_user else 0
        count = await self.db.remove_warning(user_id=user_id, reason="Ручное снятие предупреждения", actor_id=actor_id)
        await update.effective_message.reply_text(f"✅ Предупреждение снято. User={user_id}, count={count}")
    ...
    async def cmd_unmute(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        user_id = await self._extract_target_user(update, context)
        if not user_id:
            await update.effective_message.reply_text("Использование: /unmute <user_id|@username> или ответом на сообщение")
            return

        await self._unmute_user(update, user_id, "Ручное снятие мута")
    ...
    async def cmd_unban(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        user_id = await self._extract_target_user(update, context)
        if not user_id:
            await update.effective_message.reply_text("Использование: /unban <user_id|@username> или ответом на сообщение")
            return

        await self._unban_user(update, user_id, "Ручной разбан")
    ...
    async def _unmute_user(self, update: Update, user_id: int, reason: str) -> None:
        chat_id = update.effective_chat.id if update.effective_chat else self.settings.public_chat_id
        await update.get_bot().restrict_chat_member(
            chat_id=chat_id,
            user_id=user_id,
        ...
        actor_id = update.effective_user.id if update.effective_user else 0
        await self.db.clear_mute(user_id, reason, actor_id)
        await update.effective_message.reply_text(f"🔊 Мут снят: {user_id}")

    async def _unban_user(self, update: Update, user_id: int, reason: str) -> None:
        chat_id = update.effective_chat.id if update.effective_chat else self.settings.public_chat_id
        await update.get_bot().unban_chat_member(chat_id=chat_id, user_id=user_id, only_if_banned=True)
        actor_id = update.effective_user.id if update.effective_user else 0
        await self.db.clear_ban(user_id, reason, actor_id)
        await update.effective_message.reply_text(f"♻️ Пользователь разбанен: {user_id}")
    ...
    async def on_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if self._is_assistant_chat_allowed(update):
            reply = await self._build_assistant_reply(update, text)
            if reply:
                await msg.reply_text(reply)

    async def _build_assistant_reply(self, update: Update, text: str) -> str:
        items = await self.db.fetch_member_analytics(days=7)
        top_candidates = await self.db.top_scored_candidates(days=7)
        recent_messages = await self.db.fetch_recent_messages(update.effective_chat.id, limit=20)
        ...
        recent = [
            f"@{row.get('username') or row.get('user_id')}: {_short(str(row.get('text') or ''), 120)}"
            for row in recent_messages[:12]
        ]

        prompt = (
            "Ты ассистент кланового Telegram-бота для Car Parking Multiplayer 2. "
            "Отвечай по-русски. Коротко, ясно, по делу. "
            "Если вопрос про аналитику, используй только данный контекст. "
            "Если контекста не хватает, так и скажи без выдумок.\n\n"
            f"Чат: {update.effective_chat.title or update.effective_chat.type}\n"
            f"Пользователь: @{update.effective_user.username or update.effective_user.full_name}\n"
            f"Сообщение: {text}\n\n"
            f"Топ активных за 7 дней: {top_members}\n"
            f"Топ анкет за 7 дней: {top_apps}\n"
            "Последние сообщения чата:\n"
            + "\n".join(recent)
        )

        fallback = "Не смог собрать ответ. Сформулируй запрос короче: например, 'дай краткий отчет', 'кто выпал', 'что по заявкам'."
        return await self.hermes.ask_assistant(prompt, fallback)
    ...
    def register_handlers(self, app: Application) -> None:
        ...
        app.add_handler(CommandHandler("unwarn", self.cmd_unwarn))
